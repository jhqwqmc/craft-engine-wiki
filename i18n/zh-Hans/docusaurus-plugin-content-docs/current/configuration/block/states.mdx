...
用 `blueprint` 指定一个 Blockbench `.bbmodel` 文件（路径相对于配置文件所在目录，后缀可省略）。生成资源包时，CraftEngine 会把它转换成原版模型 JSON，并将内嵌贴图导出为 PNG。

```yaml
state:
  auto_state: note_block
  blueprint: "models/rocket"                     # <配置目录>/models/rocket.bbmodel
  model: "minecraft:block/custom/rocket"         # 可选：生成模型的输出路径
```

不写 `model` 时，输出路径按 bbmodel 文件名生成到 `block/` 目录下。嵌套写法同样支持：
