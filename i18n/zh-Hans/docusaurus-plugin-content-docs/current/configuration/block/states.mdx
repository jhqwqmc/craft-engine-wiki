...
<Details summary="① 仅 path — 已有现成模型文件">
```yaml
model:
  path: "minecraft:block/custom/my_block"
```
...
<Details summary="② path + generation — 自动生成模型">
```yaml
model:
  path: "minecraft:block/custom/my_block"
  generation:
...
<Details summary="③ path + textures — 只关心纹理，插件推断模型结构">
```yaml
model:
  path: "minecraft:block/custom/my_block"
  textures:
...
<Details summary="④ 仅 texture — 单张纹理，最简形式">
```yaml
model:
...
<Details summary="⑤ models 列表 — 多个模型加权随机">
```yaml
models:
...
<Details summary="⑥ transparent — 移除原始模型">
...
<Details summary="⑦ blueprint：Blockbench .bbmodel 源文件">
<Highlight color="#CC7A00">**实验性**</Highlight>

`blueprint` 用于指定 `blueprint` 文件夹中的 Blockbench `.bbmodel` 文件。该文件夹与 `configuration`、`resourcepack` 同级，配置路径相对于该目录，后缀可以省略。生成资源包时，CraftEngine 会将文件转换为原版模型 JSON，并导出其中内嵌的 PNG 贴图。
...
不写 `model` 时，输出路径按 bbmodel 相对 `blueprint` 文件夹的路径生成到 `block/` 目录下。嵌套写法同样支持：
...
内嵌贴图会导出为 PNG 并保存在模型同路径下；引用已有文件的贴图保持原引用。
...
| `blueprint`          | 字符串    | Blockbench `.bbmodel` 源文件，生成资源包时转换为模型 JSON（也可写在外观层级，与 `model` 同级） |
| `x`、`y`、`z`          | 整数     | 绕轴旋转角度，必须为 90 的倍数（`z` 需要 1.21.11+）                               |
| `uvlock`             | 布尔     | 是否锁定纹理方向，默认为 `false`                                             |
| `weight`             | 整数     | 仅在 `models` 列表中使用，被选用的权重，默认为 `1`                                 |
...
<Details summary="① 最简单：只定义属性，不定义外观（用默认外观）">
```yaml
blocks:
  default:palm_log:
    states:
      ...
      appearances:
        ...
        axisZ:
          ...
          model:
            x: 90
            path: "minecraft:block/custom/palm_log_horizontal"
            # generation: # 可以保留，但多个配置使用相同 path 和不同 generation 时会发生冲突
            #   parent: "minecraft:block/cube_column_horizontal"
            #   textures:
            #     end: "minecraft:block/custom/palm_log_top"
            #     side: "minecraft:block/custom/palm_log"
...
<Details summary="③ 用 textures 简化形式">
```yaml
blocks:
  default:palm_log:
    states:
...
<Details summary="④ 使用每个外观对应的 `entity_renderer` 进行渲染">
...
外观名称可以自定义，但同一方块内不能重复。**第一个定义的外观将作为默认外观**：当内部状态没有匹配任何变体的 `appearance` 时，系统会自动使用它。
