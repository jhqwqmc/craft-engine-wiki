...
单个原版方块可用的状态数量有限。例如，绊线最多只有 128 种状态；在保留其原版功能的情况下，最多只能腾出 127 种状态用于自定义方块。实体渲染器会将其中一种状态设为透明，再在方块中心显示展示实体，从而突破模型状态数量的限制。

:::warning

某些原版方块使用硬编码渲染器。如果模型尺寸与完整方块不一致，可能出现透视效果，如下图所示。
...
要使用展示实体，请在 `state` 或 `appearances` 配置段中添加 `entity_renderer`。
...
如果未指定 `type`，插件会根据其他选项按以下顺序尝试推导类型：
- 包含 `text` → `text_display`
- 包含 `item` → `item_display`
- 包含 `block` → `block_display`
...
以下参数适用于 `item_display`、`text_display` 和 `block_display` 三种渲染元素类型。
...
`display_transform`（也可写作 `display_context`）决定物品模型的变换方式。大多数情况下保持默认为 `none` 即可，如需使用手持、掉落在地面等变换的物品模型，可指定对应模式。

更多参数参见[展示实体通用参数](#展示实体通用参数)

<Details summary="完整示例">
```yaml
entity_renderer:
  type: item_display
  item: default:sofa
...
background_color: 64,0,0,0          # 背景颜色 (A,R,G,B)，默认为 0x40000000（半透明黑）
text_opacity: -1                    # 文本的不透明度 (0-255)，默认为 -1
...
```yaml
type: item
item: default:sleeper_sofa         # 必填：物品命名空间 ID
position: 0.5,0.5,0.5              # 可选：位置 (x,y,z)，默认方块中心
```

物品实体会自动带有旋转动画，适合简单的物品展示场景。

<Details summary="带有颜色提供器的示例">
```yaml
entity_renderer:
  type: item
...
```yaml
type: armor_stand
item: default:bench                 # 必填：物品命名空间 ID
position: 0,0.2,0                   # 可选：位置 (x,y,z)，默认方块中心
yaw: 0                              # 可选：绕 Y 轴旋转
pitch: 0                            # 可选：绕 X 轴旋转
scale: 1                            # 可选：缩放，默认为 1
small: false                        # 可选：是否小盔甲架，默认为 false
...
yaw: 0                              # 可选：绕 Y 轴旋转
pitch: 0                            # 可选：绕 X 轴旋转
sight_trace: true                   # 可选：是否对可见性进行视线追踪，默认为 true
```
...
entity_culling:
  ...
  aabb_expansion: 0.5                    # 包围盒扩展量，默认为 0.5
