...
`settings` 配置段定义方块的**基础物理属性** —— 硬度、声音、可燃性、光照、挖掘工具、活塞反应等，是方块的"材质档案"，与所挂的[行为](./behaviors.mdx)无关。每一项都**可选**（省略即用默认值），并可通过 [variant](./states.mdx) 按状态覆盖。

本页分两组：**稳定设置**（完全支持）与**不稳定设置**（尽力而为，每项注明其注意事项）。
