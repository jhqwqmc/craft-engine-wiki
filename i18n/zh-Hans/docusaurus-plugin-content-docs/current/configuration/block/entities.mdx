...
方块实体渲染器的着色源来自方块实体本身，这需要一个有效的方块行为——例如 `tint_source_block` 或 `display_item_block`。
对于 99% 的配置，你只需按照以下配置，即可为渲染的元素添加着色。
...
当然，你也可以通过 API 注册着色源。一个展示元素支持多个着色源，只要你正确指定其索引（即支持色源的方块行为的顺序）。
...
下图展示了一个配置了三个 `display_item_block` 行为组合的方块。它同时拥有三个鸭舌帽元素，每个鸭舌帽分别使用三个沙发的着色。
