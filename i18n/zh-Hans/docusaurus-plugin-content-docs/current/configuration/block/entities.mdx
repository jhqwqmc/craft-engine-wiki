...
方块实体渲染器的颜色提供器来自方块实体本身，这需要一个有效的方块行为——例如 `tint_source_block` 或 `display_item_block`。
对于 99% 的配置，你只需按照以下配置，即可为渲染的元素添加着色颜色。
...
当然，你也可以通过 API 注册颜色提供器。一个展示元素支持多个颜色提供器，只要你正确指定其着色索引（即支持颜色提供器的方块行为的顺序）。
...
下图展示了一个配置了有三个 `display_item_block` 行为的一个方块。它同时拥有三个渲染的鸭舌帽元素，每个鸭舌帽分别使用三个沙发的 `dyed_color` 组件信息作为着色颜色。
