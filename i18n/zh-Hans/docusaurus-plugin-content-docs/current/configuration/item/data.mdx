...
`data` 设置物品堆叠上的**数据组件**——名称、lore、属性、附魔、食物属性等模型和行为之外的一切。`data` 下的大多数键直接映射到[原版数据组件](https://zh.minecraft.wiki/w/数据组件)，CraftEngine 自动处理跨版本转换。
...
:::caution
`data` 内的 `custom_model_data` 仅设置纯整数，**不会绑定模型**。要关联模型定义，请在[物品根级](models#根字段)配置。
:::
...
:::caution
`data` 内的 `item_model` 仅设置组件值，**不会生成模型**。要关联模型定义并触发生成，请在[物品根级](models#根字段)配置。
:::
...
### written_book_content（成书内容 / 1.20.5+）

设置成书的内容——标题、作者、版本与页面。需要物品材质为 `written_book`（成书）。
  ...
  demo:guide_book:
    ...
    data:
      written_book_content:
        title: "生存指南"             # 字符串简写形式
        author: XiaoMoMi
        generation: 0                 # 0 原著，1 原稿的副本，2 副本的副本，3 破烂不堪
        resolved: false               # 文本是否已解析
        pages:
          - "单行页面"
          - - "第一行"
            - "第二行"
          - raw: "<red>高级页面"
            filtered: "过滤后的页面"   # 对象形式，可选启用聊天过滤的版本
```

`title` 同样支持 `raw` / `filtered` 对象形式。文本支持 MiniMessage 格式，`<arg:...>` 占位符会基于构建上下文（如查看物品的玩家）解析。

### random_values（随机值）

在构建物品时生成随机值，并将其持久化到物品上。已有的随机值会被保留，重新构建或更新物品时不会再次生成。每个条目都支持任意[数字格式](../../reference/number_format.mdx)写法。
...
随机生成的值可以在物品作为上下文参数的任何位置通过链式参数读取，例如 `<arg:item_in_hand.random.attack>`；也可以在动态属性修饰符的数值表达式中使用 `item.random.attack` 引用。

### function（执行函数）

在构建物品时执行一个或多个[通用函数](../../reference/functions.mdx)。这个处理器本身不会添加物品组件，主要用于提前准备 context 变量，让后续物品处理器复用。列表推荐使用 `functions`；只有一个函数时，也可以使用别名 `function`。

处理器按照 YAML 中的顺序运行，并共享同一个物品构建上下文。请将 `functions` 放在它需要读取的数据之后、所有使用变量的处理器之前。变量只在本次构建流程中有效：如果读取变量的名称或 lore 位于 `client_bound_data`，对应的 `functions` 也必须放进 `client_bound_data`。

下面的物品会在服务端保存三个相互独立的 `0~1` 属性随机值。物品发送给玩家时，函数处理器根据不同权重计算武器评分，并且只把最终结果提供给客户端侧 lore：
...
items:
  demo:rolled_blade:
    ...
    client_bound_data:
      # 伤害占 50%，攻速占 30%，耐久占 20%，最终得到 0~100 分。
      functions:
        - type: set_variable
          name: score
          value_type: int
      ...
      insert_lore:
        position: TAIL
        lore:
          - "<!i><gray>武器评分：<gold><var:score>"
```

三个权重之和为 `100`，因此 `ROUND(...)` 会把加权结果转换为 `0~100` 的整数评分。`client_bound_data` 内部的顺序不可颠倒：`functions` 必须先于 `insert_lore` 执行，而且两者必须位于同一个配置段中，才能共享同一份 context。`position: TAIL` 会把评分拼接到已有 lore 的尾部。支持的变量类型见 [`set_variable`](../../reference/functions.mdx#设置变量)。
...
手持源物品并运行 [`/ce debug item-component [组件] [json/snbt]`](../../reference/commands.mdx#debug-item-component)。点击结果将其复制，然后把完整的 `(json) ...` 或 `(snbt) ...` 字符串直接作为该组件的值。请保留格式前缀。
...
**按玩家动态显示。** 未使用 `client_bound_data` 时，同一物品对所有玩家显示相同内容；启用后，可以根据解锁状态、个人属性或倒计时，为每名玩家生成不同的客户端显示。

**性能更好。** 服务端的物品不再扛着大段大段的描述文本。好处有三：

- **加快 `getItemMeta` 调用。** 其他插件读取物品信息时，无需反序列化仅用于客户端显示的组件。
- **降低内存占用。** 即使容器中存有大量物品，服务端也只需保留精简数据，复杂的描述内容由渲染层处理。
- **降低区块加载开销。** lore 等展示数据仅在发送给玩家时于 Netty 线程序列化；区块加载阶段只需读取少量物品数据。
...
# custom_model_data——仅在没有 CustomModelData 时设置
...
`override_data` 的格式和处理器与 `data` 完全相同，但处理结果会写入原版物品的默认原型。此选项仅对原版物品 ID 生效。

这个功能适合移除会干扰自定义系统的原版组件。例如，移除钻石胸甲自带的属性修饰符后，原版护甲值与盔甲韧性便不会再影响自定义属性系统：
...
`override_data` 对原版物品默认原型的修改只发生在服务端，不会同步到客户端。不配合 [`client_bound_data`](#-客户端绑定数据) 时，服务端可能已经移除组件，客户端却仍显示原版属性或提示文本。条件允许时，建议用 `client_bound_data` 同步处理客户端可见的数据。

:::

除了移除组件，也可以覆写组件内容。下面修改胡萝卜的默认食物数据：
