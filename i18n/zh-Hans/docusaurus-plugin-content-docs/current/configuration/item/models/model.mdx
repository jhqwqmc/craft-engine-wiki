...
指定要渲染的物品模型，以及各着色索引使用的颜色。`type` 默认为 `minecraft:model`。
...
`blueprint` 指向 `blueprint` 文件夹中的 Blockbench `.bbmodel` 文件。该文件夹与 `configuration`、`resourcepack` 同级；路径相对于 `blueprint`，后缀可以省略。生成资源包时，CraftEngine 会将其转换为原版模型 JSON，并保留几何体、展示变换和 GUI 光照等信息。
...
**贴图：** `.bbmodel` 中内嵌的贴图会导出为 PNG，并保存在与模型同路径的 `textures` 目录中（如 `demo:furniture/rocket_v2.png`；多张贴图依次添加 `_1`、`_2` 等后缀）。引用已有文件的贴图则保留原路径，不会复制文件。
