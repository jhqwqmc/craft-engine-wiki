...
`blueprint` 指向一个 Blockbench `.bbmodel` 文件（路径相对于配置文件所在目录，后缀可省略）。生成资源包时，CraftEngine 会把它转换成原版模型 JSON——几何、展示变换、GUI 光照等一并保留，省去手动从 Blockbench 导出的麻烦。
...
指定 `path` 可以控制生成的模型 JSON（以及导出的贴图）保存到哪里；不写则按 bbmodel 文件名生成到 `item/` 目录下：
