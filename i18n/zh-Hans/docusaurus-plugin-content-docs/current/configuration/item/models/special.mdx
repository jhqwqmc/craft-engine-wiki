...
| `base` | 资源路径 | 是* | — | 用于变换、粒子纹理和 GUI 光照的模型路径。*设置 `blueprint` 时可选——此时作为生成模型的输出路径 |
| `blueprint` | string | 否 | — | 转换为基础模型 JSON 的 Blockbench `.bbmodel` 文件 <Highlight color="#CC7A00">**实验性**</Highlight>（见 [Blueprint](model.mdx#blueprintblockbench-实验性)） |
