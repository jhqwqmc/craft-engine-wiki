...
demo:bow:
  material: bow
  blueprint:
    - models/bow             # 待机 —— <配置目录>/models/bow.bbmodel
    - models/bow_pulling_0   # 拉弓 0
    - models/bow_pulling_1   # 拉弓 1
    - models/bow_pulling_2   # 拉弓 2
```

`.bbmodel` 后缀可省略。生成模型的路径默认取各文件名并置于 `item/` 下；也可以用 `models` 列表（槽位顺序一致）自行指定输出路径：
...
单槽位材质（`paper`、`diamond_sword`……）只需一个文件：`blueprint: models/rocket`。内嵌贴图会导出为 PNG 并保存在模型同路径下；引用已有文件的贴图保持原引用。详见 [Blueprint](models/model.mdx#blueprintblockbench)。
