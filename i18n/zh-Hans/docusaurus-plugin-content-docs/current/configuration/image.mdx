...
图片系统会将 PNG 映射为字体字符，使其能够出现在各种文本中。最简配置分为两步：

**第一步**：在任意包的 `configuration/` 目录中创建图片配置：
...
`<image:命名空间:id>` 标签会在文本中插入图片对应的字符。该字符绑定到指定字体，因此 Minecraft 渲染文本时会显示为图片。

---

接下来将分别介绍如何配置图片、在不同场景中使用图片，以及组合多张图片来构建复杂的 UI。

## 图片配置

### 基本字段

图片配置写在任意包的 `images:` 节点下，每张图片以 `命名空间:id` 作为唯一标识。
...
| ascent | 默认 `height - 1` | 基线偏移量。增大该值会让图片在屏幕上往上移。 |
| font | 默认 `命名空间:default` | 图片所属的字体。 |
| char | 自动分配 | 映射到此图片的码位。 |
...
`ascent` 控制字形相对于文本基线的位置。值越大，图片越靠上。

如果图片需要继续上移，但 `ascent` 已等于 `height`，无法再增加（受 `ascent ≤ height` 规则限制），可以在 PNG 图片**底部**增加透明像素。这样既能增大 `height`，也能为 `ascent` 留出更多调整空间。
...
如果不想手动分配字符，可以使用 `grid_size` 让插件自动处理：
...
可选的 `format` 参数会在图片字符前添加 MiniMessage 样式，通常用于消除阴影或调整颜色。
...
每个 shift 值都对应一张预先调整好 `height` 和 `ascent` 的图片，用于产生精确的像素位移：
...
仅在必要场景（如原版语言文件）使用 `minecraft:default`，此时务必保持 `illegal-characters-filter` 开启。
:::
...
原版语言文件（`assets/minecraft/lang/*.json`）始终使用 `minecraft:default` 渲染，因此其中的图片必须使用该字体：
