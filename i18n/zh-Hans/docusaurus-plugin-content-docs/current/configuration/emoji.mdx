...
表情依赖图片配置，请先阅读[🖼️ 图片](./image.mdx)。
:::

## 快速入门

表情由关键词触发。玩家输入关键词后，CraftEngine 会将其替换为指定的图片和文本。最简配置分为两步：
...
| `image` | 否 | 指向图片中某个格子的引用，格式为 `命名空间:id:行:列`。单格图片直接写 `命名空间:id` 即可。 |
...
如果聊天插件支持自定义字体（如 TrChat），建议将表情图片的 `font` 设为自定义字体，而不是 `minecraft:default`，以免玩家直接输入原始字符。
