...
:::note
多状态方块 —— 可以有朝向、含水、生长阶段等，需要额外配置 `properties`、`appearances`、`variants`。具体参考[🔣 方块状态](./block/states.mdx#多状态方块)。
:::
...
就像这样继续添加配置 —— `loot` 管掉落物，`events` 管交互，还可以在 `state` 里添加 [`entity_renderer`](states/entity_renderer) 选项做自定义渲染。每个选项各司其职，可以按需自由组合添加。
