    ...
    - target: minecraft:zombie
      ...
      effects:
        ...
        - type: potion_effect
          ...
          # duration 的单位是游戏刻；amplifier 从 0 开始
          duration: 60
          amplifier: 1
          ambient: false
          particles: true
...
所有数值字段都支持[数字格式](../../reference/number_format.mdx)，包括 `<attacker_attr:...>` 和 `<victim_attr:...>` 属性表达式。执行效果时，`<arg:damage>` 表示公式写回后、当前时刻的 Bukkit 结算伤害快照。
...
| 字段 | 默认值 | 说明 |
|---|---:|---|
| target | victim | victim、attacker/causing_entity 或 direct_entity |
| potion_effect | 必填 | 药水效果 ID |
| duration | 20 | 持续时间，单位为游戏刻 |
| amplifier | 0 | 从零开始的效果等级 |
| ambient | false | 是否为环境效果 |
| particles | true | 是否显示粒子 |
| show_icon | true | 是否显示客户端 HUD 图标 |

每一种已注册效果都支持通用的 `conditions` 和 `functions` 字段。系统会先判断所有条件；条件全部通过后，先执行伤害效果，再按配置顺序执行函数。函数与效果共享同一个伤害上下文，可以读取其中的实体、位置、伤害数值和属性。
