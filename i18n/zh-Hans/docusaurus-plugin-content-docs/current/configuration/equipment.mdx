...
<details>
  ...
  答：CraftEngine 预见了这种情况。解决方案出乎意料地简单（至少对 CraftEngine 而言）—— 我们通过应用特殊纹饰图案来保持原版锁链甲外观，这依托于 CraftEngine 独有的 `client-bound-data` 特性实现。你可以通过执行指令 `/ce resource enable legacy_armor` 来修复原版锁链甲。
