...
注意：若两个模板存在相同配置项，下方的模板会覆盖上方配置。但当配置项为列表类型时，它们将合并为一个列表。
...
合并功能会深度整合两个配置部分，包含所有列表——无一遗漏。本质上，合并功能的工作原理几乎完全等同于使用多个模板。
...
模板参数是按顺序应用的。这意味着您刚刚定义的参数可以立即在后续的任何参数中使用。
...
虽然大多数参数只是简单的字符串或数字，但某些场景下，更友好的选项会带来更好的体验。扩展参数类型正是为此类情况而创建的。
