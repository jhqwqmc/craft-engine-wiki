...
CraftEngine 提供了丰富的自定义选项，但逐项编写复杂配置容易产生大量重复内容。模板系统允许先定义基础结构，再通过参数和覆写复用配置，从而缩短文件并减少重复操作。
...
`namespace:template_id` 下方的内容可以自由定义，只需符合 YAML 语法。

:::

## 多模板

将 `template` 写成列表即可组合多个模板。
...
可以在模板中使用 `${xxx}` 定义参数，例如 `${nutrition}` 或 `${saturation}`；调用模板时，再通过 `arguments` 设置参数值。示例如下：
...
模板参数会按顺序应用，因此前面定义的参数可以立即用于后续参数。
...
此外还有两个特殊参数 `${__NAMESPACE__}` 和 `${__ID__}`。它们会从当前非模板配置的命名空间 ID 中自动获取，并传递给所有被引用的模板。例如：
...
在占位符名称末尾加上 `^` 或 `^^`，可对解析后的值进行大小写转换：

| 写法 | 效果 |
|------|------|
| `${arg}` | 原样输出 |
| `${arg^}` | 每个单词首字母大写（其余字符不变）。以 `_` 或空格分隔的单词会转为空格分隔。 |
| `${arg^^}` | 全部转为大写 |
...
修饰符在参数值解析完成后应用，因此也适用于其他参数类型或默认值产生的结果。

:::tip

大小写修饰符使用 JVM 默认语言环境。如果需要按指定 locale 转换，请使用 `to_upper_case` / `to_lower_case` 扩展参数类型。
...
例如，CraftEngine 自身的破坏进度纹理（你打破方块时看到的裂痕动画）就是用配置工厂生成的——10 个阶段，相同结构，只改阶段编号：
...
`${namespace:-"minecraft"}` 设置了默认值——如果实例中没有提供 `namespace`，则回退为 `minecraft`。每个实例只需指定 `stage`，一份蓝图即可生成 10 个物品。
