...
生成一个随机数。同一上下文内，相同 `id` 始终返回第一次 roll 出的值；不同 `id` 各自独立 roll。
...
分布在首次 roll 时绑定。同一上下文内如果同一个 `id` 以不同分布出现，只有第一次生效。
...
由 `<random:xxx>` 标签（或带 `id` 的 `random` 条件）roll 出的命名随机值，可以在同一上下文内通过链式参数读取：

```yaml
<arg:random.xxx>  # 读取此上下文中已 roll 出的命名随机值
```
