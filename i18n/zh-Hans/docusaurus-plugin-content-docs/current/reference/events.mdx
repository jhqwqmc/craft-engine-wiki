...
# 以op身份执行这个指令，风险自担
as-op: false # 可选; [默认值: false]
# 有些自定义指令插件基于事件驱动，使用此选项可能可以兼容
as-event: false # 可选; [默认值: false]
```
...
pitch: <arg:position.pitch> # 默认值: <arg:position.pitch>
yaw: <arg:position.yaw> # 默认值: <arg:position.yaw>
...
furniture-id: "default:bench"
x: <arg:furniture.x> # 默认值: <arg:furniture.x>
y: <arg:furniture.y> # 默认值: <arg:furniture.y>
z: <arg:furniture.z> # 默认值: <arg:furniture.z>
pitch: <arg:furniture.pitch> # 默认值: <arg:furniture.pitch>
yaw: <arg:furniture.yaw> # 默认值: <arg:furniture.yaw>
anchor-type: ground # 可选; 枚举[ground,ceiling,wall]
drop-loot: true # 默认值: true
play-sound: true # 默认值: true
```
