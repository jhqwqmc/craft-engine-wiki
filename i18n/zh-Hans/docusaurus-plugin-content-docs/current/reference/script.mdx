...
CraftEngine 内置 JavaScript 脚本系统。脚本可以被 YAML 引用（作为[函数](events.mdx#js)和[条件](conditions.mdx#js)），也可以订阅 Bukkit 事件、注册周期任务、提供自定义 [PlaceholderAPI 变量](../compatibility/placeholderapi.mdx#cejs)。
...
分别注册为 `%cejs_random_number%` 和 `%rel_cejs_relation%`。参数传递与玩家注入详见 [PlaceholderAPI 兼容](../compatibility/placeholderapi.mdx#cejs)。
