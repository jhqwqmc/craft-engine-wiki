...
items:
  default:test:
    template: mmoitems:rarity_item
    arguments:
      rarity: common
      name: "Test Item"
templates:
  mmoitems:rarity_item:
    template: mmoitems:rarity_item/internal
    arguments:
      rarity.uppercase:
        type: to_upper_case
        value: "${rarity}"
        locale: en
  mmoitems:rarity_item/internal:
    data:
      item-name: "<!i><white><image:${rarity}> ${name}"
      nbt:
        MMOITEMS_ITEM_RARITY: ${rarity.uppercase} # I don't use MMOItems so I'm not sure if 
```
