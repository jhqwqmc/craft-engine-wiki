...
Append `^` or `^^` to a placeholder name to transform the resolved value:

| Syntax | Effect |
|--------|--------|
| `${arg}` | No transformation. |
| `${arg^}` | Capitalize each word (first character upper-cased, the rest unchanged). Words separated by `_` or space are joined with spaces. |
| `${arg^^}` | Convert the whole value to upper case. |

```yaml
arguments:
  part: chainmail_helmet

# ${part}   -> chainmail_helmet
# ${part^}  -> Chainmail Helmet
# ${part^^} -> CHAINMAIL_HELMET
```

The modifier is applied after the value is resolved, so it also works on values produced by other argument types or default values.

:::tip

Case modifiers use the JVM default locale. If you need locale-specific conversion, use the `to_upper_case` / `to_lower_case` argument types instead.
...
By configuring a config factory, you can instantiate a large number of configurations. This is especially suitable for creating a series of configurations that share the same characteristics.

For example, CraftEngine's own destroy progress textures (the cracking animation when you break a block) are generated with a config factory — 10 stages, identical structure, only the stage number changes:
...
config_factory:
  ...
  blueprint:
    items:
      ${namespace:-"minecraft"}:destroy_stage_${stage}:
        data::item_name: destroy stage ${stage}
        model:
          path: item/custom/destroy_stage_${stage}
          generation:
            parent: block/cube_all
            textures:
              all: block/custom/destroy_stage_${stage}
```

`${namespace:-"minecraft"}` sets a default value — if `namespace` isn't provided in the instance, it falls back to `minecraft`. The only thing each instance specifies is `stage`, and 10 items are generated from a single blueprint.
