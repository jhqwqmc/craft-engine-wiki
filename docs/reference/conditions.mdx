...
Checks how many pieces of a specified [equipment set](../configuration/attribute/equipment-set.mdx) the player or entity in the current context is wearing.
...
The result is `false` if no player or entity is available in the condition context, or if the entity is not tracked. To match zero pieces, explicitly use `count: 0`. Specifying only `set` means “at least one piece.”
...
Check whether the context player has unlocked a recipe. Returns `false` when the context has no player. Use [`discover_recipe`](functions.mdx#discover_recipe) to unlock a recipe.
...
Evaluates a JavaScript function as the condition (must return a boolean). See [<u>📜 Scripting</u>](script.mdx) for the full scripting guide.
