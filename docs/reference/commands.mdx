...
Defaults to `config` when no argument is given. Default permission: `ce.command.admin.reload`.
...
Execute a named workflow from `resource-pack.workflows`, such as `/ce workflow default`. Tab completion lists loaded workflows. Default permission: `ce.command.admin.pack_workflow`.
...
Display the cached resource pack preferences for an online player. Omit `player` to inspect yourself; the console must specify a player. Default permission: `ce.command.debug.pack_states`.
