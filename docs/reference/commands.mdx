...
Defaults to `config` when no argument is given.

| Argument | Behavior |
|:---|:---|
| `config` | Reload configuration, including workflow definitions, triggers, and steps. |
| `recipe` | Reload configuration and recipes, including workflow definitions. |
| `host` | Reload configuration and resource pack hosting settings, including `resource-pack.packs` and `resource-pack.self-host`. |
| `pack` | Execute the currently loaded workflows whose `trigger` contains `reload_pack`. |
| `all` | Reload configuration, recipes, hosting settings, and workflow definitions, then execute the `reload_pack` workflows. |

After editing `resource-pack.workflows`, use `/ce reload` or `/ce reload config` to apply the changes. `/ce reload pack` uses the loaded definitions; it does not reread the configuration. Pack generation, upload, and sending depend on the steps in the triggered workflows.

### workflow

`/ce workflow [name]`

Execute a named workflow from `resource-pack.workflows`, such as `/ce workflow default`. Tab completion lists loaded workflows.

See [Resource Pack Workflows](../getting_start/workflows) for step fields and complete examples, and [Resource Pack Hosting](../getting_start/set_up_host) for pack definitions and delivery.

The workflow runs its configured steps in order, even if it has no `trigger`. A trigger can be a string or a list, for example `trigger: reload_pack` or `trigger: [reload_pack]`.

Uploading alone does not resend packs: add a `send_pack` step to send updates to online players currently using the specified pack. The step's `pack` value must match an ID under `resource-pack.packs`.

### pack

`/pack [enable/disable/reset] [pack-id]`

Save your own resource pack preference. `pack-id` refers to an entry under `resource-pack.packs`.

- `enable`: explicitly enable the pack.
- `disable`: explicitly disable the pack.
- `reset`: remove the override and use the pack's configured `default` value.

Player-only command. Requires `ce.resourcepack.pack.<pack-id>`; tab completion lists permitted packs. Administrators use `feature pack-preference` below to change another player's preferences.

For an online player, changing a configured pack's preference resends their selected packs. Preferences for IDs not configured on this server are still saved and cached, without resending packs. Offline players' preferences are saved for their next pack selection.
...
Apply a preset from `resource-pack.presets` to yourself. Player-only command.
...
After `/ce reload`, use `/pack preset high` or `/pack preset low`. IDs refer to `resource-pack.packs`. Packs in the selected preset are enabled; other IDs configured in `packs` or any preset are disabled. Unrelated saved preferences are preserved. An empty list disables all managed packs.

Requires `ce.resourcepack.preset.<name>` to use the whole preset. Tab completion lists permitted presets. Administrators use `feature pack-preset` below to apply a preset to another player.

Preferences are saved together. Online players receive one update when their selection changes; offline players use the saved preferences on their next connection. Applying a preset copies its selections: later edits to the preset require applying it again.
...
`/ce item component add [component] [snbt]`

Add or replace a data component on the main-hand item (player only, 1.20.5+).

Use a component ID such as `minecraft:damage`; tab completion lists component IDs registered on the server. The value must be valid SNBT for that component on your server version. Enter the component's value directly, without a `(snbt)` prefix or an enclosing component-ID map. Strings, numbers, lists, and compounds are accepted according to the component's format, including values containing spaces.
...
Unknown components and invalid values produce an error without applying the change. Adding an existing component replaces its entire value; compound values are not merged.

To set an item model on 1.21.2+, use `/ce item component add minecraft:item_model "minecraft:diamond_sword"`. Custom model data can be set through `minecraft:custom_model_data` using the SNBT format supported by your server version.

### item component remove

`/ce item component remove [component]`

Remove the specified data component from the main-hand item, including a component supplied by the item type's defaults (player only, 1.20.5+).
...
`/ce item component reset [component]`

Restore the specified component to the item type's default value (player only, 1.20.5+). If the item type has no default for that component, it is removed. This resets only the named component, not the whole item or a previous command's value.

```text
/ce item component reset minecraft:damage
```

All three component commands require a non-empty main hand and support component-ID tab completion. They replace the old `item custom-model-data`, `item item-model`, and `item remove-component` commands, which are no longer available.

## Feature Commands

### feature pack-preference

`/ce feature pack-preference [player] [enable/disable/reset] [pack-id] (--silent)`

Set the selected players' pack preferences. Requires `ce.command.admin.pack_preference`; individual pack permissions are not required. The target is required and accepts an online player name or a player selector, such as `@a` or `@p`.

### feature pack-preset

`/ce feature pack-preset [player] [preset] (--silent)`

Apply a preset to the selected players. Requires `ce.command.admin.pack_preset`; individual preset permissions are not required. The target is required and accepts an online player name or a player selector, such as `@a` or `@p`.

### feature send-pack

`/ce feature send-pack [player]`

Resend the selected resource packs to the specified player(s), using their saved preferences and each pack's `default` setting. Packs are sent in configured order; unavailable packs cannot be sent. This command does not generate or upload packs.
...
`/ce debug pack-states (player)`

Display the cached resource pack preferences for an online player. Omit `player` to inspect yourself; the console must specify a player.

| State | Color | Meaning |
|:---|:---|:---|
| `enabled` | Green | Explicitly enabled by the player's saved preference. |
| `disabled` | Red | Explicitly disabled by the player's saved preference. |
| `unset` | Yellow | No saved override; the pack's configured `default` applies. |

The output includes all saved pack IDs, even those not configured on this server. Such pack names have a strikethrough, for example ~~old_pack~~, while their saved state remains visible. Configured packs without an override are also listed. If preferences have not loaded yet, the command reports that instead.

These are preference states, not confirmation that the client downloaded or loaded a pack.
...
Serializes the specified data component of the held item as JSON or SNBT (player only, 1.20.5+). The result includes the `(json) ` or `(snbt) ` prefix required by CraftEngine; click the result to copy the complete string to your clipboard.

For example, `/ce debug item-component minecraft:custom_name snbt` returns a value ready to paste into an exact recipe predicate or an item's direct component configuration.
