...
### toggle-entity_culling

`/ce feature toggle-entity_culling [player] (state)`

This command is used to toggle whether entity culling is enabled for a player.

### entity_culling-distance-scale

`/ce feature entity_culling-distance-scale set [player] [scale]`
...
This command is used to create a new resource. If no namespace is specified, the pack will be used as the namespace.
...
### get-block_state-registry-id

`/ce debug get-block_state-registry-id [real_block_state]`
