...
Set a variable, use \<arg:var_\[name\]\> to access it (replacing \[name\] with the actual variable name)

```yaml
type: set_variable
name: my_variable # required; string
number: 1.0 # optional; number
as_int: false # optional; boolean; default: false
...
Play the totem animation. Similar to `play_totem_animation`, but the `volume` and `pitch` values are not clamped.
...
id: "minecraft:ender_pearl" # required; string; only vanilla items are supported
