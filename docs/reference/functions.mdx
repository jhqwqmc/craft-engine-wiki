...
Spawn a particle, For specific parameters, see [✨ Particle Effects](./particle_effect)
...
Set a variable on the current execution context. Functions that run later with the same context can read it with [`<var:name>`](text_format.mdx#var_name_).
...
| --- | --- | --- |
| int | Integer; numeric results are floored | [Number Format](number_format.mdx) |
| double | Double | [Number Format](number_format.mdx) |
...
```yaml
# Proxy plugin message — by server name (BungeeCord / Velocity)
type: transfer
server: "lobby" # required if host not set; string
```

```yaml
# Direct transfer — by host + port
type: transfer
...
Invokes a JavaScript function. See [<u>📜 Scripting</u>](script.mdx) for the full scripting guide.
