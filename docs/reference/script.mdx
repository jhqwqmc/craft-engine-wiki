...
CraftEngine ships with a JavaScript scripting system. Scripts can be referenced from YAML (as [functions](events.mdx#js) and [conditions](conditions.mdx#js)), subscribe to Bukkit events, register recurring tasks, and provide custom [PlaceholderAPI placeholders](../compatibility/placeholderapi.mdx#cejs).
...
Registered as `%cejs_random_number%` and `%rel_cejs_relation%`. See [PlaceholderAPI compatibility](../compatibility/placeholderapi.mdx#cejs) for details on arguments and player injection.
