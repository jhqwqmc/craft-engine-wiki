...
Drops the exact item the player used to place the furniture, preserving all its data (custom name, durability, NBT, etc.). If the furniture was placed via API without a source item, the `item` fallback is used instead.

:::tip

Always use `furniture_item` as your entry type for furniture loot. This ensures the player gets back their original placement item rather than a fresh copy.
...
References another loot table and generates its loot inline. When no CraftEngine loot table with that id exists, it falls back to the datapack loot table with the same id, so vanilla tables can be referenced directly.
