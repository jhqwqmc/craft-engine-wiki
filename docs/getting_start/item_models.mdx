...
> ⚠️ Model texture dimensions must be powers of two—for example, 16×16, 32×48, or 64×64. Textures that do not meet this requirement cause the client to downgrade mipmapping.
...
CraftEngine infers the parent from the material: `golden_sword` → `handheld` (tool), `paper` → `generated` (flat icon), block items → `cube_all`. **You don't need to specify a parent, write a model block, or touch CustomModelData.**
...
When `texture:` isn't enough — you want a different parent or display transforms — use the `generation` format.
...
> ⚠️ Filenames must follow [namespace path rules](./project_structure.mdx#whats-a-namespace): lowercase letters, digits, `-`, `_`, `.`, `/` only. Uppercase or special characters will cause load failures.
