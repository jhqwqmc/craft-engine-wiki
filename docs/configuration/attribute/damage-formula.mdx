...
```yaml
formula:
  # Formula type. Default: expression
  type: expression
  # Expression returning the new damage. Required
...
`composition` divides damage into named parts and adds their evaluated results together. Each part may itself be a string, an `expression`, a nested `composition`, or `js`.
