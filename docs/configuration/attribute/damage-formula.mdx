...
All numeric fields accept a [Number Format](../../reference/number_format.mdx), including `<attacker_attr:...>` and `<victim_attr:...>` expressions. During effect execution, `<arg:final_damage>` is a snapshot of the currently resolved damage immediately after the formula is applied.
