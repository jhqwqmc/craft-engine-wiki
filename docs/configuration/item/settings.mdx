...
Marks an item as part of an equipment set while it is equipped in one of the specified slots.

```yaml
equipment_set_part:
  - slots:                  # Slots in which this item counts as a set piece
      - head
    sets:                   # Equipment set IDs
      - example:potion_test
```

An item can belong to multiple sets. Set tiers do not accumulate; only the highest satisfied tier applies.
