...
Determines the break power of this item when used as a tool. A block with [`required_break_power`](../block/settings.mdx#required_break_power) can only be effectively mined by tools whose break power is greater than or equal to the required value. (Default: inherit from the item's vanilla material)
...
See [Attribute → Item Attribute Modifiers](../attribute/modifier.mdx) for all slots, scopes, dynamic values, and the random weapon example.
...
An item can belong to multiple sets. `slots` accepts only `mainhand`, `offhand`, `head`, `chest`, `legs`, `feet`, `body`, and `saddle`; slot groups are not accepted. Set tiers do not accumulate, and only the highest satisfied tier applies. See [Attribute → Equipment Sets](../attribute/equipment-set.mdx) for the complete configuration.
...
`amount` and `percent` can be combined: each material restores `amount + percent × max durability` points of durability.
...
The item keeps its last point of durability instead of being destroyed. An item in this "broken" state can no longer be used: attacking, mining, shooting bows/crossbows and right-click usages (e.g. stripping logs, hoeing dirt, raising a shield) are all blocked until it is repaired. Worn armor is automatically unequipped when it breaks — moved into the player's inventory, or dropped on the ground if the inventory is full.
...
![](/img/item_settings_6.png)
![](/img/item_settings_7.png)
