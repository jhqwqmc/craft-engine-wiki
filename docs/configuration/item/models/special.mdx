...
| `base` | asset path | Yes* | — | Namespaced model path for transforms, particle texture, and GUI light. *Optional when `blueprint` is set — becomes the output path of the generated model |
| `blueprint` | string | No | — | Blockbench `.bbmodel` file converted into the base model JSON <Highlight color="#FFA500">Experimental</Highlight> (see [Blueprint](model.mdx#blueprint-blockbench-experimental)) |
