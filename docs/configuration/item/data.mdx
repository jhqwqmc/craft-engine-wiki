...
[Premium] Use `client_bound_material` + `client_bound_data.block_state` for items that need a different server-side material.
:::
...
Your own components — not maintained or version-checked by CraftEngine. Format may break across Minecraft updates.
