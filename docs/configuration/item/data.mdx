...
For performance considerations, more complex configuration is required when we need to dynamically split a line of lore into multiple lines. Below is the advanced lore configuration format, which is more intricate but offers greater flexibility. The "priority" determines the final position of the text segment—the lower the priority, the closer it will be to the top.
