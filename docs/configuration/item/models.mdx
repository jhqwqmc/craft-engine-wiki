...
<Details summary="item_model & custom_model_data" defaultOpen={false}>
...
| Field | Version | Format | How it works |
|-------|---------|--------|-------------|
| `custom_model_data` | All | Integer | Attached to the item stack. The resource pack's model JSON `overrides` list matches this number to a model path. |
| `item_model` | 1.21.2+ | Namespaced ID | Points directly to `assets/<ns>/items/<id>.json`. No override lookup needed. |
...
|----------|------------|
| You need a **specific** CustomModelData number (e.g. another plugin references it) | `custom_model_data: <number>` |
...
|---------|---------|--------|
| `item.always-use-item-model` | `true` | Generate `item_model` even when `custom_model_data` is present. Recommended. |
| `item.always-use-custom-model-data` | `false` | Force CustomModelData allocation even for 1.21.2+-only packs. |
| `item.always-generate-model-overrides` | `false` | Force legacy overrides even for 1.21.4+-only packs (useful for Bedrock converters). |
| `item.custom-model-data-starting-value.default` | `10000` | First auto-allocated CustomModelData number. |
| `item.custom-model-data-starting-value.overrides.<material>` | — | Per-material override, e.g. `paper: 20000`. |
| `item.client-bound-model` | `true` | <Highlight color="#f2184eff">Premium</Highlight> Keep CustomModelData and item_model client-side, enabling dynamic updates without affecting existing items. |
...
<Highlight color="#FFA500">Experimental</Highlight>

If you'd rather model in Blockbench than write JSON, point `blueprint` at one `.bbmodel` file **per model slot** — CraftEngine converts each file into a vanilla model JSON (textures included) and builds the same model tree it would have built from textures.

```yaml
demo:bow:
  material: bow
  blueprint:
    - bow                   # Idle — <pack>/blueprint/bow.bbmodel
    - bow_pulling_0         # Pulling 0
    - bow_pulling_1         # Pulling 1
    - bow_pulling_2         # Pulling 2
```

The `.bbmodel` extension is optional, and paths are relative to the `blueprint` folder (sibling of `configuration` and `resourcepack`). Generated model paths default to `item/` plus each file's path relative to the `blueprint` folder; pass `models` (in the same slot order) to pick the output paths yourself:
...
Single-slot materials (`paper`, `diamond_sword`, …) take just one file: `blueprint: rocket`. Embedded textures are exported as PNGs alongside the model; textures referencing existing files keep their original reference. See [Blueprint](models/model.mdx#blueprint-blockbench-experimental) for the details.
...
:::tip
`blueprint` also works in `legacy_model` and each of its `overrides` — a `.bbmodel` file is converted and used as that model, with `path` as the optional output location. See [Blueprint](models/model.mdx#blueprint-blockbench-experimental).
:::
