...
Single-slot materials (`paper`, `diamond_sword`, …) take just one file: `blueprint: rocket`. Embedded textures are exported as PNGs alongside the model; textures referencing existing files keep their original reference. See [Blueprint](models/model.mdx#blueprint-blockbench) for the details.
...
:::tip
`blueprint` also works in `legacy_model` and each of its `overrides` — a `.bbmodel` file is converted and used as that model, with `path` as the optional output location. See [Blueprint](models/model.mdx#blueprint-blockbench).
:::
