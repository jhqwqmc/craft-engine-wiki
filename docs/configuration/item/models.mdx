...
demo:bow:
  material: bow
  blueprint:
    - models/bow             # Idle — <config dir>/models/bow.bbmodel
    - models/bow_pulling_0   # Pulling 0
    - models/bow_pulling_1   # Pulling 1
    - models/bow_pulling_2   # Pulling 2
```

The `.bbmodel` extension is optional. Generated model paths default to each file's name under `item/`; pass `models` (in the same slot order) to pick the output paths yourself:
...
demo:bow:
  ...
  blueprint:
    - models/bow
    - models/bow_pulling_0
    - models/bow_pulling_1
    - models/bow_pulling_2
  models:
    - demo:item/bow
...
Single-slot materials (`paper`, `diamond_sword`, …) take just one file: `blueprint: models/rocket`. Embedded textures are exported as PNGs alongside the model; textures referencing existing files keep their original reference. See [Blueprint](models/model.mdx#blueprint-blockbench) for the details.
