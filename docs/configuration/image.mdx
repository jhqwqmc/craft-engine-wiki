...
You can also use the `grid_size` option to let the plugin automatically arrange the corresponding number of characters for you, without needing to manually specify `chars`.
...
"Referencing" allows you to assign an alias to a specific image within a multi-character sprite sheet.
...
You can use a very simple command `/ce debug image [id]` to preview the effect of the image.
...
## Using in Language Files

Sometimes we create images to replace parts of the titles of vanilla containers, but the vanilla language file does not support custom fonts. In such cases, you need to set the font of the image to `minecraft:default`, otherwise it will not work.
