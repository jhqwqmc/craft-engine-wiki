...
Point `blueprint` at a Blockbench `.bbmodel` file (relative to the config file — the extension is optional), and CraftEngine converts it into a vanilla model JSON at pack generation, exporting embedded textures as PNGs.
...
Leave `model` out, and the output path falls back to the bbmodel's file name under `block/`. The nested map form works the same way:
