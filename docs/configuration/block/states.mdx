...
### Visual States
The visual representation of a block is controlled by either the `auto_state` or `state` option. You can choose one of them based on your precision requirements.

#### 🛠️ Using `auto_state`: Let the Plugin Choose for You

The parameter of `auto_state` points to a predefined set of visual states in the plugin. These states share key characteristics, such as **identical collision boxes and consistent rendering properties** (e.g., whether transparency is supported).

- What is its purpose?

When creating a new block, we typically only care about its core characteristics (such as collision box size) and not which specific block state is used. `auto_state` was designed for this purpose: the plugin will automatically assign you an available state from the group that meets the characteristics.

- An example

For a note block, `minecraft:note_block[instrument=hat,note=0,powered=false]` and `minecraft:note_block[instrument=hat,note=1,powered=false]` are identical in terms of core characteristics. You don’t need to remember or type such long state parameters. Simply write `auto_state: note_block`, and the plugin will automatically select an available note block state for you.

:::info
Currently available `auto_state` options:
...
- weeping_vine - Any weeping vine
- twisting_vine - Any twisting vine
- cave_vine - Any cave vine
- kelp - Any kelp
- chorus - Any chorus plant
:::
...
Use the shorthand format `block_name:ID` defined in the `block_state_mappings` configuration.

> Example：`note_block:100` （This indicates using the 101st mapped note block state）
...
- **Probability Calculation**：The probability of each model being selected is determined by the ratio of its own `weight` value to the total weight of all candidate models.
...
Restriction: Must be a multiple of 90.
- z (1.21.11+)
Rotation angle of the model around the Z-axis.
Unit: Degrees (°)
Restriction: Must be a multiple of 90.
- uvlock
...
- type：Specifies the type of the property. You can view all available property types in the [ℹ️ Properties](states/properties.mdx) subpage.
...
blocks:
  default:palm_log:
    states:
      appearances:
        axisY:
          # You can also use "auto_state: note_block" here
          state: "note_block:0" 
          model:
...
- **Single Property**：The format is property_name=value, e.g., `axis=y`.
- **Multiple Properties**：Separate them with English commas, e.g., `axis=y,age=7`. The order of properties does not affect matching.
...
- **Quantity Limit**：The number of IDs cannot exceed the `serverside-blocks` limit set in `config.yml`.
