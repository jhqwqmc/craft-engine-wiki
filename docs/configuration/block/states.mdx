...
<Details summary="① path only — existing model file">
```yaml
model:
  path: "minecraft:block/custom/my_block"
```
...
<Details summary="② path + generation — auto-generate model">
```yaml
model:
  path: "minecraft:block/custom/my_block"
  generation:
...
<Details summary="③ path + textures — textures only, plugin infers model structure">
```yaml
model:
  path: "minecraft:block/custom/my_block"
  textures:
...
<Details summary="④ texture only — single texture, simplest form">
```yaml
model:
...
<Details summary="⑤ models list — multi-model weighted random">
```yaml
models:
...
<Details summary="⑥ transparent — remove original model">
...
<Details summary="⑦ blueprint — Blockbench .bbmodel source">
<Highlight color="#FFA500">Experimental</Highlight>

Point `blueprint` at a Blockbench `.bbmodel` file in the `blueprint` folder (sibling of `configuration` and `resourcepack`; paths are relative to it — the extension is optional), and CraftEngine converts it into a vanilla model JSON at pack generation, exporting embedded textures as PNGs.

```yaml
state:
  auto_state: note_block
  blueprint: "rocket"                     # <pack>/blueprint/rocket.bbmodel
  model: "minecraft:block/custom/rocket"         # Optional: output path of the generated model
```

Leave `model` out, and the output path falls back to `block/` plus the bbmodel's path relative to the `blueprint` folder. The nested map form works the same way:

```yaml
model:
  path: "minecraft:block/custom/rocket"
  blueprint: "rocket"
```

Embedded textures are exported as PNGs alongside the model; textures referencing existing files keep their original reference.
...
| `blueprint` | string | Blockbench `.bbmodel` source file — converted into the model JSON at pack generation (may also be set at the appearance level, as a sibling of `model`) |
...
<Details summary="① Simplest: define properties only, no explicit appearances (uses default)">
```yaml
blocks:
  default:palm_log:
    states:
...
<Details summary="② Different models for different directions">
```yaml
blocks:
  default:palm_log:
    states:
...
<Details summary="③ Using textures shorthand">
```yaml
blocks:
  default:palm_log:
    states:
...
<Details summary={<>④ Rendering with an <code>entity_renderer</code> per appearance</>}>
...
<Details summary="Expand full configuration">
```yaml
blocks:
  default:palm_leaves:
    states:
...
<Details summary="More variants examples">
