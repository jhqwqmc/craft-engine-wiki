...
Omitting `delay` or setting it to a non-positive value switches the behavior to **random-tick mode** — the block advances on random ticks instead of a fixed timer.
