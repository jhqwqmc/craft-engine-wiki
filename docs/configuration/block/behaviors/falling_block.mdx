...
`hurt_amount` and `max_hurt` must be configured together — both need to be positive (`> 0`) for the falling block to hurt entities. If either is `-1` or `0`, no damage is dealt.
