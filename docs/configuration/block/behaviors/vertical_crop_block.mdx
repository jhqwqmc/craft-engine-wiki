...
The crop's root keeps growing even after new segments appear above — each segment grows independently, so a tall stalk can produce new segments from multiple positions at once.
