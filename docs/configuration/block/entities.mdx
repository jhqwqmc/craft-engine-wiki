...
To configure a display entity, simply add an `entity_renderer` option. This option accepts a configuration section or list as its parameter.
...
The tint source for the block entity renderer comes from the block entity itself, which requires a valid block behavior — such as `tint_source_block` or `display_item_block`. 
For 99% of configurations, you only need to follow the setup below to apply tinting to the rendered elements.
...
Of course, you can also register a color source through the API. A display element supports multiple tint sources, as long as you correctly specify its index (i.e., the order of block behaviors that support color sources).
...
The image below shows a block configured with three `display_item_block` behaviors. It also has three cap elements, each using the tinting from one of the three sofas.
...
entity_culling:
  ...
  view__distance: 64 # View distance culling. -1 means unlimited distance.
