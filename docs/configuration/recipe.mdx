...
Predicates allow you to require additional conditions on ingredients beyond just their item ID or tag. This is useful when you want a recipe to only accept items with specific enchantments or data components.
...
<div style={{textAlign: 'center'}}>
  <img src={require('/img/recipe_15.png').default} alt="" />
</div>
...
# Exact data component match (advanced)
- type: exact
  component: minecraft:custom_name
  value: ... # NBT tag value
```

:::tip[Get the exact component value]

Hold the source item and run [`/ce debug item-component [component] [json/snbt]`](../reference/commands.mdx#debug-item-component). Click the result to copy it, then paste the complete `(json) ...` or `(snbt) ...` string into `value`. Keep the format prefix.
...
Transform processors are used by transform recipes (`smithing_transform`, `shaped_transform` and `shapeless_transform`) to control how the source item's data is merged into the result.

```yaml
transform_processors:
  # Apply data to the combined item
  - type: apply_data
    data: 
      item_name: "new name"

  # Merge the enchantments of the final product and the original item, rather than choosing one of them.
  - type: merge_enchantments

  # Keep the specific data of the custom_data component (1.20.5+)
...
The visual-result feature prevents players from directly anticipating the outcome of **crafting/smithing** recipes. This is particularly useful for recipes involving randomized items.
    ...
    - type: apply_data
      data:
        remove_components:
          - attribute_modifiers
        lore:
          - "<gray>Attack Damage 1~3"
          - "<gray>Attack Speed 1~2"
```
...
Functions can be used to run specific functions when an item is successfully crafted/smithed.
...
Conditions allow you to set requirements for this recipe, preventing players who do not meet them from using it.
...
If the player has all the ingredients for a recipe and meets its conditions, the recipe will be automatically unlocked for the player. The default value of this option can be set in `config.yml`.
...
When set to `true`, the recipe is automatically unlocked for the player when they join the server, regardless of whether they have obtained the ingredients.
...
You can also control this behavior globally in `config.yml`:

```yaml
recipe:
  unlock-on-join:
    all: false   # whether to unlock all recipes on join
    list:        # additional recipe ids to unlock on join
      - default:amethyst_torch
```
...
<div style={{textAlign: 'center'}}>
  <img src={require('/img/recipe_13.png').default} alt="" />
</div>

### Shaped Transform Recipe
...
default:topaz_crossbow_transform:
  ...
  ingredients:
    A:
      ...
      source: true  # Mark as the source item
    B: default:topaz
  result:
...
<div style={{textAlign: 'center'}}>
  <img src={require('/img/shaped_transform.png').default} alt="" />
</div>
...
<div style={{textAlign: 'center'}}>
  <img src={require('/img/recipe_14.png').default} alt="" />
</div>

### Shapeless Transform Recipe

Shapeless transform recipe works like the shapeless crafting recipe, but one ingredient must be marked as `source: true`. The source item's data will be merged into the final result, making it ideal for upgrading existing items.
  ...
  default:topaz_sword_transform:
    ...
    ingredients:
      - items: minecraft:diamond_sword
        source: true  # Mark as the source item
      - default:topaz
    # merge the source item's components into the result
    merge_components: true # default: true
    result:
      id: default:topaz_sword
      count: 1
    transform_processors:
...
default:topaz_bow:
  ...
  template_type: default:topaz # slot 1 (Optional)
  ...
  result:
    ...
    count: 1
    post_processors: [] # see the result post processor above
  transform_processors: []
```
