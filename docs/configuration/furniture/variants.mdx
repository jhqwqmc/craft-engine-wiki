...
elements:
  - item: default:bench
    ...
    brightness: # optional; If there are options where the two values below must not be -1
      block-light: 15 # required; default: -1
      sky-light: 15 # required; default: -1
    view-range: 1.0 # When the distance is more than (view-range * Entity Distance * 64) the entity is not rendered
